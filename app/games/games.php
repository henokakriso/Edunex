    ...
    public function run(): void {
        ...
        $decks = Database::all(
            "SELECT d.id, d.title, COUNT(c.id) AS card_count
             FROM ai_decks d JOIN ai_cards c ON c.deck_id = d.id
        ...
        if ($deckId) {
            $cards = Database::all(
                "SELECT front, back FROM ai_cards WHERE deck_id = ? AND user_id = ? ORDER BY RAND()", [$deckId, (int)$u['id']]
            );
        }
        ...
        $courses = Database::all(
            "SELECT DISTINCT c.id, c.title FROM courses c
             JOIN course_enrollments en ON en.course_id = c.id
