    ...
    public function run(): void {
        ...
        $courses = Database::all(
            "SELECT c.*, ce.progress, ce.completed,
             ...
             WHERE ce.user_id = ? AND c.status = 'published' $df ORDER BY ce.enrolled_at DESC", [$uid, $uid]);
        ...
        $upcomingExams = Database::all(
            "SELECT e.*, c.title AS course_title FROM exams e JOIN courses c ON c.id = e.course_id
             ...
             WHERE ce.user_id = ? AND e.status = 'published' AND e.end_time > NOW() $dfExam ORDER BY e.start_time LIMIT 5", [$uid]);
        ...
        $assignments = Database::all(
            "SELECT a.*, c.title AS course_title,
             ...
             WHERE ce.user_id = ? AND a.due_date > NOW() $dfAssign ORDER BY a.due_date LIMIT 5", [$uid, $uid, $uid]);

        $dfAnnounce = demo_filter('an');
        $announcements = Database::all(
            "SELECT an.*, u.first_name, u.last_name FROM announcements an JOIN users u ON u.id = an.author_id
             WHERE an.school_id = ? AND (an.audience = 'all' OR an.audience = 'students') $dfAnnounce
             ORDER BY an.pinned DESC, an.created_at DESC LIMIT 4", [$u['school_id']]);

        $dfAttn = demo_filter('at');
        $attendance = Database::all(
            "SELECT date, status FROM attendance at WHERE student_id = ? AND course_id IN (SELECT id FROM courses WHERE school_id = ?) $dfAttn ORDER BY date DESC LIMIT 10", [$uid, $u['school_id']]);

        $dfAttn2 = demo_filter('at');
        $attStats = Database::one(
            "SELECT COUNT(*) AS total,
             ...
             FROM attendance at WHERE student_id = ? AND date >= DATE_SUB(CURDATE(), INTERVAL 30 DAY) $dfAttn2", [$uid]);
