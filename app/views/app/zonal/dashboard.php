...
<div class="grid" style="grid-template-columns:repeat(auto-fit,minmax(160px,1fr));gap:1rem;margin-bottom:1.5rem">
  ...
  <div class="card stat"><div class="stat-num"><?= e($stats['woredas']) ?></div><div class="stat-lbl">Woredas</div></div>
...
<div class="card" style="margin-bottom:1.5rem">
  <div class="card-head"><h2>Schools in My Zone</h2></div>
  <div style="overflow-x:auto">
    <table class="table">
      <thead><tr><th>School</th><th>Woreda</th><th>Students</th><th>Teachers</th><th>Directors</th><th>Status</th></tr></thead>
      <tbody>
      <?php if (empty($schoolStats)): ?>
        <tr><td colspan="6" style="text-align:center;color:var(--muted)">No schools found.</td></tr>
      <?php else: foreach ($schoolStats as $ss): ?>
        <tr>
          <td><a href="<?= url('index.php?r=zonal/school&id='.$ss['school']['id']) ?>"><?= e($ss['school']['name']) ?></a></td>
          <td><?= e($ss['school']['woreda_name'] ?? '—') ?></td>
          <td><?= e($ss['students']) ?></td>
          <td><?= e($ss['teachers']) ?></td>
...
<div class="card">
  ...
  <?php if (empty($woredas)): ?>
    <li style="padding:1rem;color:var(--muted)">No woredas found.</li>
  <?php else: foreach ($woredas as $w): ?>
    <li style="padding:0.75rem 1rem;border-bottom:1px solid var(--border)">
      <a href="<?= url('index.php?r=zonal/woreda&id='.$w['id']) ?>"><?= e($w['name']) ?></a>
    </li>
  <?php endforeach; endif; ?>
  </ul>
</div>
