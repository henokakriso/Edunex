...
<div class="page-head">
  ...
  <button class="btn btn-primary" onclick="document.getElementById('badge-form').style.display=document.getElementById('badge-form').style.display==='none'?'block':'none'"><?= icon('plus') ?> New badge</button>
</div>
...
<form method="post" class="card" id="badge-form" style="display:none;margin-bottom:18px">
  ...
  <div class="grid2">
    ...
    <div class="flex-col"><label class="small faint">Icon</label>
      <select class="input" name="icon" id="f_icon"><?php foreach ($icons as $ic): ?><option value="<?= $ic ?>"><?= str_replace('-', ' ', ucfirst($ic)) ?></option><?php endforeach; ?></select>
    </div>
  ...
  <?php foreach ($all as $b): ?>
    <div class="card" style="padding:16px">
      <div class="flex-between" style="align-items:flex-start">
        ...
        <form method="post" class="inline" data-confirm="Delete this badge permanently?"><?= csrf_field() ?><button class="btn btn-sm btn-danger" name="delete_badge" value="<?= (int)$b['id'] ?>"><?= icon('trash') ?></button></form>
      </div>
